        ...
        try:
                ...
                for idx, stp_name in enumerate(stp_files):
                    ...
                    for f in os.listdir(folder):
                        ...
                        if os.path.isfile(fpath):
                            ...
                            if f.lower().endswith("_stp.prt") or ext == ".log":
                                try:
                                    os.remove(fpath)
                                except Exception:
                                    pass

                    # Read per-file result from export_result.json generated by the journal (preserves full structural G/D/P/T validation)
                    per_file_entry = None
                    if os.path.exists(result_json_path):
                        try:
                            with open(result_json_path, "r", encoding="utf-8") as f_res:
                                p_data = json.load(f_res)
                            if p_data.get("files") and len(p_data["files"]) > 0:
                                per_file_entry = p_data["files"][0]
                        except Exception:
                            pass

                    if per_file_entry and per_file_entry.get("status") == "success":
                        aggregated["success"] += 1
                        aggregated["files"].append(per_file_entry)
                    else:
                        aggregated["failed"] += 1
                        err_msg = per_file_entry.get("error") if per_file_entry else "Journal produced no result manifest"
            ...
            if subfolder == "DWG":
                ...
                for idx, prt_name in enumerate(prt_files):
                    ...
                    # Read per-file result from export_result.json generated by the journal
                    per_file_entry = None
                    if os.path.exists(result_json_path):
                        try:
                            with open(result_json_path, "r", encoding="utf-8") as f_res:
                    ...
                    if per_file_entry:
                        ...
                        aggregated["files"].append({
                            ...
                            "output": None,
                            "status": "failed",
                            "error": "Journal produced no result manifest"
                        })
