...
def find_nx_environment():
    ...
    if env_base and os.path.exists(env_base) and env_base not in candidates:
            ...
            if subfolder == "IGES":
                ...
                for idx, stp_name in enumerate(stp_files):
                    ...
                    if os.path.exists(result_json_path):
                        try:
                            ...
                            if (
                                ...
                                and files
                                and files[0].get("input") == stp_name
                            ):
                    ...
                    if per_file_entry and per_file_entry.get("status") == "success":
                        ...
                        aggregated["failed"] += 1
                        err_msg = per_file_entry.get("error") if per_file_entry else f"Journal produced no valid result manifest (NX return code: {returncode})"
